...
// {fact rule=log-injection@v1.0 defects=0}
                    ...
                    if (getOrganizations(authId).size() == 0) {
                        LOGGER.debug(String.format("Did not find orgs for user=%s, storing the defaults", sanitizeForLogging(authId)));
...
// {/fact}
    ...
    public static void initializeAuthenticatedSession(final Http.Request request, final String userName, final List<String> organizations) {
        ...
        LOGGER.info("added user " + sanitizeForLogging(userName) + " to session.");
    }
    ...
    private static final Pattern LOG_SANITIZE_PATTERN = Pattern.compile("[\r\n\t]");

    /**
     * Sanitizes input for safe logging by removing line breaks and control characters
     * that could be used for log injection attacks.
     *
     * @param input the input string to sanitize
     * @return sanitized string safe for logging
     */
    private static String sanitizeForLogging(final String input) {
        if (input == null) {
            return "null";
        }
