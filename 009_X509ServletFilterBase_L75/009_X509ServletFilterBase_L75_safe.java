...
// {fact rule=log-injection@v1.0 defects=0}
            ...
            if (logger.isDebugEnabled())
            {
                logger.debug("Enforcing X509 Authentication:" + sanitizeForLogging(String.valueOf(this.enforce)));
            }
...
// {/fact}
                ...
                if (logger.isDebugEnabled())
                {
                    ...
                    {
                        logger.debug("Enforcing cert-contains:" + sanitizeForLogging(this.certContains));
                    }
                }
            }
        }
            ...
            else
            {
                ...
                if (!httpRequest.isSecure())
                {
                    if (this.httpsPort != null)
                    {
                        ...
                        if (logger.isDebugEnabled())
                        {
                            logger.debug("Redirecting to:" + sanitizeForLogging(redirectUrl));
                        }
    ...
    private boolean validCert(X509Certificate[] certs)
    {
        ...
        if (name.contains(this.certContains))
        {
            if (logger.isDebugEnabled())
            {
                logger.debug("Cert: " + sanitizeForLogging(name) + "  contains:  " + sanitizeForLogging(this.certContains));
            }
        ...
        {
            logger.error("Cert: " + sanitizeForLogging(name) + "  does not contain:  " + sanitizeForLogging(this.certContains));
            return false;
        }
    }
     ...
     * Removes or replaces characters that could be used to manipulate log files.
     */
    private String sanitizeForLogging(String input)
    {
        ...
        // Remove CRLF characters and other control characters that could be used for log injection
            ...
            if (objectNames != null && objectNames.size() == 0)
            {
                ...
                if (objectNames1 != null)
                {
                    for (ObjectName objectName : objectNames1)
                    {
                        ...
                        {
                            logger.warn("Setting clientAuth=want on MBean:" + sanitizeForLogging(objectName.toString()));
