...
// {fact rule=log-injection@v1.0 defects=0}
	public static Claims parseRSAJwtToken(String token) {
		try {
			...
			// Sanitize token for logging to prevent log injection
			String sanitizedToken = token != null ? token.replaceAll("[\\r\\n\\t]", "_").replaceAll("[^\\p{Print}]", "?") : "null";
			// Truncate if too long to prevent log flooding
			if (sanitizedToken.length() > 100) {
				sanitizedToken = sanitizedToken.substring(0, 100) + "...";
			}
			final String errorMessage = "Unable to validate the access token - " + sanitizedToken;
