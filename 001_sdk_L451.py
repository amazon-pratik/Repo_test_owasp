...
class MajsoulHandler:
    ...
    # {fact rule=code-quality-metrics-cyclomatic-complexity@v1.0 defects=1}
    ...
    # {fact rule=code-quality-metrics-cyclomatic-complexity@v1.0 defects=0}
    @dump_args
...
# Following function is duplicated for annotation purpose
def dumpWebSocket2(filename='ws_dump.pkl'):
    # 监听mitmproxy当前websocket，将所有报文按顺序交由handler.parse
    server = ServerProxy("http://127.0.0.1:37247")  # 初始化服务器
    liqi = LiqiProto()
    tot = 0
...
# {/fact}
 ...
 # {fact rule=resource-leak@v1.0 defects=1}
def replayWebSocket1(filename='ws_dump.pkl'):
    # 回放历史websocket报文，按顺序交由handler.parse
    handler = MajsoulHandler()
    history_msg = pickle.load(open(filename, 'rb'))
    liqi = LiqiProto()
...
# {/fact}
 ...
 # {fact rule=code-injection@v1.0 defects=1}
def replayWebSocket2(filename='ws_dump.pkl'):
    # 回放历史websocket报文，按顺序交由handler.parse
    handler = MajsoulHandler()
    history_msg = pickle.load(open(filename, 'rb'))
    liqi = LiqiProto()
