...
public abstract class BaseController {
    ...
    /**
     * Sanitizes input for safe logging by removing CRLF characters
     * 
        ...
        try {
            ...
            if (CommonUtils.isNotEmpty(str)) {
                try {
                    ...
                    log.error("解密参数失败，key值：" + SecurityCode.getEncryptKey() + "，密文：" + sanitizeForLogging(str), e);
                    str = "{}";
// {fact rule=log-injection@v1.0 defects=0}
                }
            } else {
                log.warn("请求非法：" + sanitizeForLogging(request.getRequestURI()));
            }
        } catch (Exception e) {
            log.warn("请求非法：" + sanitizeForLogging(request.getRequestURI()));
        }
