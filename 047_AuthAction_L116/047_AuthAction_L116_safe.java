            ...
            if (currentPlayer.getId() == playerId) {
                if (!checkForBan(tregmine, currentPlayer)) {
                    ...
                    Tregmine.LOGGER.info("Restoring token " + token + " to " +
                                         sanitizeForLogging(currentPlayer.getRealName()));
                } else {
                    token = null;
                    found = false;
                    reason = "Banned";
// {/fact}
                    Tregmine.LOGGER.info("Refused to restore token for " +
                            sanitizeForLogging(currentPlayer.getRealName()) + " due to ban.");
        ...
        if (!checkForBan(tregmine, player)) {
            ...
            Tregmine.LOGGER.info("Assigned token " + token + " to " + sanitizeForLogging(player.getRealName()));
        } else {
            token = null;
            found = false;
            reason = "Banned";

            Tregmine.LOGGER.info("Denied token for " + sanitizeForLogging(player.getRealName()) +
                    " due to ban.");
        }
    }
